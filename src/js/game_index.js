...
import { Layout, Menu, Breadcrumb, Row, Col, Slider, Button, message, Modal } from 'antd';
        ...
        if(_.filter(cardsInfo, {guessed: false}).length == 0)  {
            ...
            usersList = _.map(this.state.usersList, function(userInfo, index) {
                if(userInfo.userName === userName) {
                    found = true
                    return {...userInfo, score: userInfo.score > this.state.score ? userInfo.score : this.state.score}
                } else {
                    return userInfo
                }
            })
...
return (
    <Layout style={{height: "100vh"}}>
        ...
        <Content style={{ padding: '0 50px', marginTop: 64  }}>
        {
          ...
          <Button type="primary" style={{ float: 'left', margin: '20px' }} onClick={() => message.info('Enjoy the game!')}>Welcome! {this.state.userName}</Button></div>
            )
            : null
        }
          <div style={{ background: '#fff', padding: 24, minHeight: 380  }}>
    {this.state.selectScoreBoard ? <ScoreBoard usersList={this.state.usersList} /> : this.state.word ? <GameStart selectedCard={(card, index) => this.selectedCard(card, index)} cardsInfo={this.state.cardsInfo} selectedIndex={this.state.selectedIndex} /> : <GameInitial wordEntered={this.setStatus} />}
    </div>
    </Content>
